...
def importance_weighted(nif, prior, n_importance_samples=64, name='importance_weight'):
    ...
    def _forward(nif_outputs, prior_outputs, batch_depth=1):
        ...
        assert prior_outputs['log_det'].ndim == 1
        w = prior_outputs['log_det'] - logsumexp(prior_outputs['log_det'])
        w = jnp.exp(w)
        ...
        objective = jnp.sum(w*(nif_outputs['log_pxgz'] + prior_outputs['log_det']))

        # Compute the upper bound on the log likelihood log_p(x) + KL[p(z|x)||q(z|x)]
        assert nif_outputs['log_qzgx'].ndim == 1
        log_px_upper_bound = jnp.sum(w*prior_outputs['log_det']) + jnp.mean(nif_outputs['log_det'])
        # log_px_upper_bound = jnp.sum(w*(nif_outputs['log_pxgz'] + prior_outputs['log_det'] - nif_outputs['log_qzgx']))

        # Compute the lower bound on the log likelihood log_p(x) - KL[q(z|x)||p(z|x)]
        assert nif_outputs['log_det'].ndim == 1
        log_px_lower_bound = jnp.mean(prior_outputs['log_det'] + nif_outputs['log_det'])

        # Compute the estimate of the log likelihood
        log_px_estimate = logsumexp(prior_outputs['log_det']) - jnp.log(n_importance_samples)
    ...
    def forward(params, state, inputs, **kwargs):
        ...
        nif_outputs, updated_nif_state = nif_apply(params['nif'], state['nif'], inputs, reverse=False, key=k1, n_importance_samples=n_importance_samples, **kwargs)
    ...
    def init_fun(key, inputs, batched=False, batch_depth=1, **kwargs):
        ...
        nif_outputs, ni_flow = nif(k1, inputs, batched=batched, batch_depth=batch_depth, n_importance_samples=n_importance_samples, **kwargs)
        if(batched == True):
            batch_depth += 1
        else:
            batched = True
        ...
        # outputs = _forward(nif_outputs, prior_outputs, batch_depth=batch_depth)
