...
class Softmax():
  ...
  def __call__(self, x, params=None, inverse=False, rng_key=None, **kwargs):
    ...
    if inverse == False:
      ...
      v = jax.scipy.special.logsumexp(x, axis=-1)

      log_det = jnp.log(w).sum(axis=sum_axes)

      # Evaluate the likelihood of v
      _, llc = self.v_flow(v, params=self.flow_params, rng_key=rng_key, inverse=False, **kwargs)

      return w, llc + log_det + 0.5*jnp.log(x.shape[-1])
    else:
      w = x
      log_w = jnp.log(w)
      log_det = log_w.sum(axis=sum_axes)
      ...
      z = log_w + v
      return z, llc + log_det + 0.5*jnp.log(x.shape[-1])
...
if __name__ == "__main__":
  ...
  flow = nux.Sequential([Softmax(v_flow=nux.UnitGaussianPrior()),
                         nux.DirichletPrior()])
  ...
  truth_flow = nux.Sequential([nux.NonlinearCoupling(n_layers=3,
                                                     working_dim=32,
                                                     hidden_dim=64,
                                                     nonlinearity=util.square_swish,
                                                     dropout_prob=0.0,
